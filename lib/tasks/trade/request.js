...
class Request {
     ...
     */
    getUser() {
        let sellRegexReverse = new RegExp("BTW", "gi")
        let buyRegexReverse = new RegExp("STW", "gi")

        // Find username
        this.user = this.message[0].replace(":", "")
        this.message.shift()
        this.message = this.message.join(" ").split("-").join(" ").split(" ")

        // User ends with Offer identifier? Remove it starting from the end
        if (this.user.toLowerCase().endsWith("wts")) {
            console.log("does end with WTS")
            this.user = this.user.split("").reverse().join("").replace(sellRegexReverse, "").split("").reverse().join("")
            this.message.unshift("WTS")
        }
        if (this.user.toLowerCase().endsWith("wtb")) {
                        console.log("does end with WTB")
            this.user = this.user.split("").reverse().join("").replace(buyRegexReverse, "").split("").reverse().join("")
            this.message.unshift("WTB")
        }
    }
    ...
    getValue() {
        for (let j = 0; j < this.offers.length; j++) {
            ...
            // Detect values
            // NOTE: this.message[offer.index + i] equals offer.message,
            ...
            for (let i = 0; i < offer.message.length && this.message[offer.index + i]; i++) {
                 ...
                 */
                let inspect = (index => {
                    return this.message[offer.index + index]
                })
                ...
                else if (containsDecimal && parsedInt > 5 && parsedInt < 100000) {
    ...
    cleanOffers() {
        for (let offer of this.offers) {
            ...
            offer.subMessage = offer.message ? offer.message.join(" ") : ""
