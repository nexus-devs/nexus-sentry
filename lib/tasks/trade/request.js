...
class Request {
        ...
        for (let partition of this.partitions) {
            ...
            for (let i = 0; i < partition.message.length; i++) {
                ...
                for (let component of partition.item.components) {
                    let cwords = component.name.split(" ")
    ...
    addSingleComponent(partition, component, offer, i) {
        ...
        if (word.toLowerCase().includes(component.name.toLowerCase())) {
            offer.component = component.name
    ...
    cleanOffers() {
        for (let offer of this.offers) {
            offer.region = process.env.NEXUS_SENTRY_REGION || "NA"
