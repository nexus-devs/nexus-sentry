...
class ChatSentry {
    ...
    readChat() {
        return new Promise((resolve, reject) => {
            ...
            // Bot was previously offline -> tell server we're back up
            else if (this.awaitUpstatus) {
                this.client.post("/warframe/v1/bots/upstatus", {
                    status: "online",
                    region: process.env.NEXUS_SENTRY_REGION
                })
                this.awaitUpstatus = false
            }
        })
    }

    isOffline() {
      if (new Date - this.lastRequest > 60000) {
        this.awaitUpstatus = true
        return true
      } else {
        return false
      }
