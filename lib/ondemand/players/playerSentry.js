            ...
            setTimeout(() => {
                postOpen.read().then(text => {
                    ...
                    lines.forEach((line, index) => {
                        console.log("comparing: " + line + " & " + username + " -> " + line.toLowerCase().fuzzy.score(username.toLowerCase(), 0.5))
                        if (line.toLowerCase() === (username.toLowerCase())) {
                            this.player.name = lines[index]
                            return resolve()
                        }
                        else if (line.toLowerCase().fuzzy.score(username.toLowerCase(), 0.5) > 0.6) {
