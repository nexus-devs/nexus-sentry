            ...
            setTimeout(() => {
                postOpen.read().then(text => {
                    ...
                    lines.forEach((line, index) => {
                        console.log("comparing: " + line.toLowerCase() + " & " + username + " -> " + line.toLowerCase().score(username.toLowerCase(), 0.5))
                        if (line.toLowerCase() === (username.toLowerCase())) {
                            this.player.name = lines[index]
                            return resolve()
                        }
                        else if (line.toLowerCase().score(username.toLowerCase(), 0.5) > 0.6) {
